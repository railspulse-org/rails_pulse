...
module RailsPulse
  module Api
    module V1
      # The endpoints an extension adds answer 402 without it, so the CLI
      ...
      class ExtensionControllerTest < ActionDispatch::IntegrationTest
        ...
        test "every extension endpoint has a stub route" do
          assert_equal ExtensionController::FEATURES.keys.sort, PATHS.keys.sort
        end

        test "answers 402 with the feature and a message" do
          PATHS.each do |feature, path|
            get instance_exec(&path), headers: HEADERS
            body = JSON.parse(response.body)

            assert_response :payment_required, feature
            assert_equal "requires_extension", body["error"]
            assert_equal feature, body["feature"]
            assert_includes body["message"], "provided by an extension"
            assert_nil body["url"]
          end
        end
