...
**Zeitwerk uses the host's inflections.** A host with `inflect.acronym "SQL"` expects `sql_query_normalizer.rb` to define `SQLQueryNormalizer`. Any file under `app/` whose basename contains a common acronym (sql, csp, http, api, json, …) must be pinned in `ACRONYM_SAFE_INFLECTIONS` in `lib/rails_pulse/engine.rb`; `test/lib/rails_pulse/zeitwerk_test.rb` eager-loads with `SQL`, `CSP` and `API` declared to catch omissions.
