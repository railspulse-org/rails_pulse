...
`exe/rails-pulse` requires only `lib/rails_pulse/cli/main.rb`, never the engine: the CLI (`RailsPulse::CLI`, Thor) and the MCP server (`RailsPulse::Mcp::Server`, stdio, `lib/rails_pulse/mcp/`) run on a developer's machine and reach the app through `CLI::Client` over HTTP, with the URL and token from `RAILS_PULSE_URL`/`RAILS_PULSE_TOKEN` or `~/.rails-pulse` (`CLI::Config`, `RAILS_PULSE_CONFIG` to relocate). A 402 becomes `CLI::Client::ProRequiredError`; the CLI prints it and exits 1, `Mcp::Tools::Helpers#respond` returns it as data (`requires_pro: true`) so the agent relays it instead of retrying. `rails-pulse install claude` copies `lib/rails_pulse/cli/agent_files/claude_skill.md` to `~/.claude/skills/rails-pulse/SKILL.md`. Decision 0018.
