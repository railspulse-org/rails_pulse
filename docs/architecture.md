...
`app/controllers/rails_pulse/api/v1/` serves read-only JSON under the engine mount at `api/v1/`: routes, requests, queries, jobs, job_runs and deployments, each an index with `limit`/`offset`/`since`/`until` and a `{ data, meta }` envelope built by the serializers in `app/serializers/rails_pulse/api/v1/`. `Api::V1::BaseController` skips the dashboard authentication and accepts only `config.api_token` (`secure_compare`; no token means 401 for everything). The five endpoints only `rails_pulse_pro` can answer (alerts, alert_rules, summary, threshold_suggestions, setup) are drawn as stubs on `Api::V1::ProController` that return 402 with the feature name; `config/routes.rb` skips them when `RailsPulse.pro?`, and the Pro engine appends the real ones. `DeploymentsController#deployment_regressions` is the hook Pro overrides to attach regression checks.

`exe/rails-pulse` requires only `lib/rails_pulse/cli/main.rb`, never the engine: the CLI (`RailsPulse::CLI`, Thor) and the MCP server (`RailsPulse::Mcp::Server`, stdio, `lib/rails_pulse/mcp/`; needs the host's Gemfile to add the `mcp` gem, which is only a development dependency here) run on a developer's machine and reach the app through `CLI::Client` over HTTP, with the URL and token from `RAILS_PULSE_URL`/`RAILS_PULSE_TOKEN` or `~/.rails-pulse` (`CLI::Config`, `RAILS_PULSE_CONFIG` to relocate). A 402 becomes `CLI::Client::ProRequiredError`; the CLI prints it and exits 1, `Mcp::Tools::Helpers#respond` returns it as data (`requires_pro: true`) so the agent relays it instead of retrying. `rails-pulse install claude` copies `lib/rails_pulse/cli/agent_files/claude_skill.md` to `~/.claude/skills/rails-pulse/SKILL.md`. Decision 0018.
