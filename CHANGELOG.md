...
- **JSON API, `rails-pulse` CLI and MCP server.** A read-only, token-authenticated API under `/rails_pulse/api/v1`, a `rails-pulse` executable for the terminal and CI, and an MCP server (`rails-pulse mcp`, twelve tools; add `gem "mcp"` to your Gemfile) for Claude Code, Codex, Cursor and other clients. Tools that read Rails Pulse Pro data say what Pro would add when it is not installed instead of failing.
