...
- **Summary aggregation writes each period in a handful of statements.** Every route, query and job summary used to be found and saved individually, so an hour with a few hundred routes and queries cost over 700 SQL statements; rows are now upserted in bulk against the summaries unique index. The transaction also now runs on the Rails Pulse connection rather than the host's primary database on separate-database installs.
